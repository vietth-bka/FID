...
class FaceModel_teacher(torch.nn.Module):
    ...
    def forward(self, data, label):
        feat, intermediate_feat = self.backbone.forward(data)
        arcface_outputs, cos_theta_teacher = self.head.forward(feat, label)

        return arcface_outputs, cos_theta_teacher, intermediate_feat
...
class FaceModel(torch.nn.Module):
    ...
    def __init__(self, backbone_factory, head_factory):
        ...
        self.adaptor = nn.Conv2d(64, 64, 1, bias=False)
    ...
    def forward(self, data, label):
        feat, intermediate_feat = self.backbone.forward(data)
        arcface_outputs, cos_theta_student = self.head.forward(feat, label)
        projected_feat = self.adaptor(intermediate_feat)
        return arcface_outputs, cos_theta_student, intermediate_feat
...
def train_one_epoch(data_loader, model, teacher_model, optimizer, lr_schedule, criterion, cur_epoch, conf, anneal_alpha):
    ...
    for batch_idx, (images, labels) in enumerate(data_loader):
        ...
        with torch.no_grad():
            _, teacher_ori_logits, feat_teacher = teacher_model.forward(images, labels)

        kd_loss = loss_fn_kd(ori_logits, labels, teacher_ori_logits, conf)
        mse_loss = loss_Euc(projected_feat, feat_teacher, conf)
        loss = (1. - conf.alpha)*cse_loss + 100*kd_loss + 100*mse_loss
        # loss = 100*kd_loss
        ...
        if batch_idx % (len(data_loader)//4) == 0:
            lr = get_lr(optimizer)
            logger.info('Epoch %d, iter %d/%d, lr %f, total_loss %.5f, cse_loss %.5f, kd_loss %.10f, mse_loss %.7f, acc %.3f' % 
                        (cur_epoch, batch_idx, len(data_loader), lr, loss_meter.avg, cseloss_meter.avg, kdloss_meter.avg, mseloss_meter.avg, acc_meter.avg))
...
def train(conf):
    ...
    # student model 
    backbone_factory = get_model('r100',dropout=0.2, fp16=False)
    head_factory = HeadFactory(conf.head_type, conf.head_conf_file)
    student_model = FaceModel(backbone_factory, head_factory)
    ...
    #                                                                                                             , map_location='cuda:1')
    student_state_dict = torch.load('/media/v100/DATA2/vietth/insightface-master/recognition/arcface_torch/glint360k_r100_fp16/backbone.pth',
                                                                                                                  map_location = 'cuda:1')
    ...
    teacher_state_dict = torch.load('/media/v100/DATA2/vietth/FaceX-zoo/out_dir_old/BEST_r100_exXaug_ArcFace.pt', map_location='cuda:1')
    ...
    for epoch in range(ori_epoch, conf.epoches):
        ...
        if test_loss < best_loss:
            ...
            saved_name = f'BEST_logits_{conf.alpha}_lr{conf.lr}_r100S_r100T_KD.pt'
            # saved_name = f'BEST_logits_mix_lr{conf.lr}_r100S_{conf.teacher}T_noAdapt_KD.pt'
            state = {
