...
from utility import loss_fn_kd, cosineAnnealing
import math
...
def train_one_epoch(data_loader, model1, model2, optimizer1, optimizer2, criterion, cur_epoch, conf, anneal_alpha):
    ...
    for batch_idx, (images, labels) in enumerate(data_loader):
        ...
        kd_loss1 = loss_fn_kd(logits1, labels, logits2, conf)
        kd_loss2 = loss_fn_kd(logits2, labels, logits1, conf)

        loss = cse_loss1 + cse_loss2 + 100*(kd_loss1 + kd_loss2)
        ...
        size = images.shape[0]
        cse_meter1.update(cse_loss1.item(), size)
        cse_meter2.update(cse_loss2.item(), size)

        kd_meter1.update(kd_loss1.item(), size)
        kd_meter2.update(kd_loss2.item(), size)
        
        acc_meter1.update(acc1, size)
        acc_meter2.update(acc2, size)
        
        # update per-epoch meter
        avg_cseMeter1.update(cse_loss1.item(), size)
        avg_cseMeter2.update(cse_loss2.item(), size)
...
def evaluate_one_epoch(data_loader, model1, model2, criterion, conf, mode, cur_epoch):
    ...
    for batch_idx, (images, labels) in enumerate(data_loader):
        ...
        loss_meter1.update(loss1.item(), images.shape[0])
        loss_meter2.update(loss2.item(), images.shape[0])
...
def train(conf):
    ...
    # student model 
    resnet100 = get_model('r100', dropout=0.4, fp16=False)
    resnet50 = get_model('r50', dropout=0.2, fp16=False)
    head_factory = HeadFactory(conf.head_type, conf.head_conf_file)
    model1 = FaceModel(resnet50, head_factory)
    model2 = FaceModel(resnet100, head_factory)
    ...
    std1 = torch.load('/media/v100/DATA4/thviet/FaceX-zoo/out_dir_old/BEST_r50_exXaug_ArcFace.pt', 
                                                                                                map_location=conf.device)
    model1.load_state_dict(std1)

    std2 = torch.load('/media/v100/DATA4/thviet/FaceX-zoo/out_dir_old/BEST_r100_exXaug_ArcFace.pt', 
                                                                                                map_location=conf.device)
    ...
    for epoch in range(ori_epoch, conf.epoches):
        ...
        if test_loss < best_loss:
            ...
            saved_name = f'BEST_dml_backbone_T{int(conf.temperature)}_sync.pt'
            state = {
